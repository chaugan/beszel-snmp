...
export default memo(function PM25Chart({ chartData }: { chartData: ChartData }) {
	...
	const newChartData = useMemo(() => {
		...
		for (let data of chartData.systemStats) {
			...
			let keys = Object.keys((data.stats as any)?.pm25 ?? {})
			for (let i = 0; i < keys.length; i++) {
				let key = keys[i]
				newData[key] = (data.stats as any).pm25[key]
			...
			<ChartContainer className={cn("h-full w-full absolute aspect-auto bg-card opacity-0 transition-opacity", { "opacity-100": true })}>
				<LineChart accessibilityLayer data={newChartData.data} margin={chartMargin}>
					...
					<YAxis
						...
						width={72}
						domain={[0, "auto"]}
						tickFormatter={(val: number) => `${toFixedFloat(val, 0)} µg/m³`}
							// using 3 for superscript may render as ³ in most fonts
						tickLine={false}
						axisLine={false}
					/>
					{xAxis(chartData)}
					<ChartTooltip
						...
						content={
							<ChartTooltipContent
								...
								contentFormatter={(item: any) => `${decimalString(item.value)} µg/m³`}
							/>
						}
					/>
					{colors.map((key) => (
