...
import { cn, formatShortDate, toFixedFloat, chartMargin, decimalString } from "@/lib/utils"
...
export default memo(function HumidityChart({ chartData }: { chartData: ChartData }) {
	...
	/** Format humidity data for chart and assign colors */
	const newChartData = useMemo(() => {
		...
		for (let data of chartData.systemStats) {
			...
			let keys = Object.keys(data.stats?.h ?? {})
			for (let i = 0; i < keys.length; i++) {
				let key = keys[i]
				newData[key] = (data.stats.h as any)[key]
