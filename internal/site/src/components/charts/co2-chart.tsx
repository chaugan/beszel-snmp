...
export default memo(function CO2Chart({ chartData }: { chartData: ChartData }) {
	...
	const newChartData = useMemo(() => {
		...
		for (let data of chartData.systemStats) {
			...
			let keys = Object.keys((data.stats as any)?.co2 ?? {})
			for (let i = 0; i < keys.length; i++) {
				let key = keys[i]
				newData[key] = (data.stats as any).co2[key]
			...
			<ChartContainer className={cn("h-full w-full absolute aspect-auto bg-card opacity-0 transition-opacity", { "opacity-100": true })}>
				<LineChart accessibilityLayer data={newChartData.data} margin={chartMargin}>
					...
					<YAxis
						...
						width={64}
						domain={[0, "auto"]}
						tickFormatter={(val) => `${toFixedFloat(val, 0)} ppm`}
						tickLine={false}
						axisLine={false}
					/>
					{xAxis(chartData)}
					<ChartTooltip
						...
						content={
							<ChartTooltipContent
								...
								contentFormatter={(item) => `${decimalString(item.value)} ppm`}
							/>
						}
					/>
					{colors.map((key) => (
